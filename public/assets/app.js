...
function renderDates(meta){
 const map=meta?.module_dates||{};const names={market:'市場',institutional:'法人',sectors:'族群',capital_flow:'資金',research:'研究池'};
...
function renderMarket(m){
 ...
 marketStatus.textContent=m?.status||'待更新';
 marketSummary.textContent=verdict==='待更新'?'目前可核對訊號不足，等待完整資料。':`市場方向初判：${verdict}`;
...
function renderCoverage(i){
 const c=i?.coverage||{};coverage.innerHTML=[
 ['上市筆數',c.twse_rows],['上櫃筆數',c.tpex_rows],['合計解析',c.parsed_rows],['全市場覆蓋',c.full_market_coverage?'是':'否']
 ].map(([a,b])=>`<div class="metric"><span class="muted">${a}</span><strong>${b??'待更新'}</strong></div>`).join('');
}
...
function renderFlow(flow,sectors){
 ...
 inflowList.innerHTML=rows.slice(0,5).map(item).join('')||'待更新';
 outflowList.innerHTML=[...rows].sort((a,b)=>(a.flow_score??0)-(b.flow_score??0)).slice(0,5).map(item).join('')||'待更新';
...
(async()=>{
 await Promise.all(['market','institutional','sectors','capital_flow','research','meta'].map(load));
 const m=state.market||{},i=state.institutional||{},s=state.sectors||{},f=state.capital_flow||{},r=state.research||{},meta=state.meta||{};
 inst=i;renderMarket(m);renderCoverage(i);renderDates(meta);renderFlow(f,s);renderResearch(r,s);renderSectors(s);renderInst('foreign_buy_top50');
